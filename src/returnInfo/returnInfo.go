...
type returnInfo struct {
	...
	State       string `json:"state"`     //true已经审批 false未审批
	Pass		string `json:"pass"`	  //审批是否通过 true通过 false否决
...
type historyinfo struct {
	Goodsinfos []returnInfo `json:"returninfo"`
}

func (g *returnInfo)  Init(shim.ChaincodeStubInterface) pb.Response{
	return shim.Success(nil)
}

func (g *returnInfo) Invoke(stub shim.ChaincodeStubInterface) pb.Response  {
...
func (g *returnInfo) goodsquery(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
...
func (g *returnInfo) goodsave(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=2 {
...
func (g *returnInfo) delete(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=1 {
		return shim.Error("except one arg")
	}else {
...
func (g *returnInfo) queryhistory(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=1 {
		return shim.Error("except one arg")
	}else {
		var goodsApplys []returnInfo
...
func (g *returnInfo) createApply(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=2 {
		return shim.Error("except two args")
	}
...
func (g *returnInfo) getQueryResult(stub shim.ChaincodeStubInterface, args []string) pb.Response{
	if len(args)!=4 {
